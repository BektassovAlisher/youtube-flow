...
![Главная страница VideoFlow](docs/screenshots/home-light.webp)
...
| 📝 **Конспект** | Разделы с таймкодами, глоссарий и главные выводы; оглавление с переходом к разделу |
| 🧠 **Проверь себя** | Вопросы для самопроверки карточками: ответ скрыт, пока его не откроешь, и строится по транскрипту |
...
| ⏱️ **Прогресс обработки** | Интерфейс показывает, какой шаг пайплайна сейчас идёт |
| 🌓 **Веб-интерфейс** | Next.js с серверным рендерингом, готовые примеры на главной, тёмная и светлая тема |
...
<table>
  ...
  <tr>
    <td><img src="docs/screenshots/home-stages.webp" alt="Шаги обработки нового видео"></td>
    <td><img src="docs/screenshots/library.webp" alt="Библиотека"></td>
  </tr>
  <tr>
    <td align="center">Шаги обработки нового видео</td>
    <td align="center">Библиотека</td>
  </tr>
  <tr>
    <td><img src="docs/screenshots/video-summary.webp" alt="Конспект с оглавлением"></td>
    <td><img src="docs/screenshots/video-quiz.webp" alt="Проверь себя: ответ по транскрипту"></td>
  </tr>
  <tr>
    <td align="center">Конспект с оглавлением</td>
    <td align="center">Проверь себя: ответ по транскрипту</td>
  </tr>
  <tr>
    <td><img src="docs/screenshots/video-script.webp" alt="Сценарий подкаста"></td>
    <td><img src="docs/screenshots/video-audio.webp" alt="Аудио подкаста"></td>
  </tr>
  <tr>
    <td align="center">Сценарий подкаста</td>
    <td align="center">Аудио подкаста</td>
  </tr>
</table>
...
│   ├── app/
│   │   ├── page.tsx, home.tsx        # Главная: ввод ссылки, шаги обработки, примеры, «Как это работает»
│   │   ├── library/page.tsx          # Библиотека (серверный компонент)
│   │   ├── videos/[id]/page.tsx      # Загрузка видео на сервере
│   │   ├── videos/[id]/view.tsx      # Вкладки: конспект, проверь себя, сценарий, аудио, рекомендации, вопросы
│   │   ├── api/[...path]/route.ts    # Прокси /api/* → FastAPI
│   │   ├── header.tsx                # Навигация и переключатель темы
│   │   └── globals.css               # Темы, типографика, анимации
│   ├── lib/api.ts                    # Типы и клиент API для браузера
│   ├── lib/server-api.ts             # Запросы к API из серверных компонентов
│   └── Dockerfile
...
| `POST` | `/generate/stream` | То же, но с прогрессом: NDJSON-строка `{"step": …}` на каждый шаг графа, в конце `{"result": …}` |
...
### Создать
- Заголовок с эффектом печати и поле для ссылки на первом экране
- Во время обработки видно, какой шаг идёт: субтитры → проверка → конспект → сценарий (`/generate/stream`)
- Готовые примеры из библиотеки — открыть конспект можно без ожидания
- «Как это работает» — карточки с превью результата каждого шага
...
### Страница видео
- Вкладки: Конспект → Проверь себя → Сценарий → Аудио → Рекомендации → Вопросы
- Оглавление конспекта сбоку; жирные термины выделены маркером
- «Проверь себя»: вопросы карточками, ответ открывается по кнопке и содержит ссылки на моменты видео
- Таймкоды в конспекте и ответах ведут на нужный момент видео
- Плеер и скачивание mp3, свои вопросы по видео, удаление видео

Данные страниц загружаются на сервере и приходят сразу в HTML — без экрана загрузки и с сохранением прокрутки при перезагрузке.
