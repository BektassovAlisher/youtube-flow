...
**Интерфейс**
- Streamlit заменён на **Next.js**: главная с эффектом печати, секция «Как это работает» с живыми превью каждого шага, библиотека карточками, страница видео со вкладками.
- **Тёмная и светлая тема**: новая тема раскрывается кругом от переключателя, выбор запоминается.
- Таймкоды `[MM:SS]` в конспекте и ответах стали ссылками на нужный момент видео.
- Аудио проигрывается и скачивается прямо на странице, в том числе в Docker.

**Пайплайн и API**
- **Кэш — первый шаг графа**: уже обработанное видео отдаётся за доли секунды без запросов к YouTube, классификатору и LLM. Ссылки `youtu.be/…`, `watch?v=…&t=…`, `shorts/…` распознаются как одно видео.
- **Бессрочное хранение** вместо удаления через 7 дней.
- Снятая с Groq модель `llama-3.3-70b-versatile` заменена на `openai/gpt-oss-120b`.
- **Аудио для любых языков**: голоса находят ведущих и по-русски, и по-английски, в том числе с `**жирными**` именами. Временные mp3 больше не копятся в корне проекта.
- **Устойчивость к ответам LLM**: классификатор, ключевые слова и критик больше не падают на неполном или невалидном JSON.
- **RAG**: индексируются только принятые видео; индекс больше не стирается при повторной обработке; путь к ChromaDB не зависит от папки запуска.
- **Ошибки**: сохранение в БД больше не проглатывается молча; некорректная ссылка даёт `400`, а не `500`.

**Безопасность и инфраструктура**
- Название видео запрашивается по `video_id`, а не по произвольному URL пользователя (защита от SSRF).
- `.dockerignore`: `.env` с ключами, `venv` и mp3 больше не попадают в Docker-образ.
- Postgres из `docker-compose` проброшен на порт `5435`, чтобы не конфликтовать с локальными базами.
- Добавлена проверка кэша: `tests/test_cache_first.py`.
...
### 1. 💾 Cache Node
Первый шаг: достаёт `video_id` из ссылки (любой формат — `watch?v=`, `youtu.be/`, `shorts/`, с таймкодом) и ищет видео в PostgreSQL. Если оно уже обработано — сразу возвращает результат, без запросов к YouTube и LLM. Видео хранятся бессрочно.
...
Интерфейс написан на **Next.js** (`web/`) по мотивам [ularkimsanov.com](https://ularkimsanov.com): Bricolage Grotesque и Inter Tight в заголовках, Inter в тексте, JetBrains Mono в подписях.
